...
//			String block = decimal.substring(Math.max(firstSome + (i - 1) * BASE_DECIMAL_DIGITS, 0),
...
//	 * 
//	 * @param digits
//	 *            a list of digits, each between 0 (inclusive) and {@link BASE}
//	 *            (exclusive).
...
public class DecimalBigInt implements Comparable<DecimalBigInt> {
	...
	public static DecimalBigInt valueOf(String decimal) {
		...
		int[] digits = new int[bigLen];
		for (int i = 0; i < bigLen; i++) {
			String block = decimal.substring(Math.max(firstSome + (i - 1) * RADIX_DECIMAL_DIGITS, 0),
	 ...
	 * <a href="http://en.wikipedia.org/wiki/Short_division">Wikipedia's article
